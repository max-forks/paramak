...
def create_reactor(
    ...
    theta = 3 * np.pi / 2
    divertor_radius = major_radius + minor_radius * np.cos(
    ...
    points = [
        (divertor_radius - divertor_thickness, -2000),
        (divertor_radius - divertor_thickness, 2000),
        (divertor_radius + divertor_thickness, 0),
        (divertor_radius + divertor_thickness, -700),
    ]
    divertor = (
...
def export_reactor_to_png(reactor, file_path):
    reactor.add(
        cq.Workplane("XZ")
        .text("Paramak", fontsize=200, distance=10)
        .translate((0, 0, -1215))
    )
    reactor.exportPNG(
        options={
...
for i in range(len(original_radial_build)):
    ...
    for factor in factors:
        ...
        reactor = create_reactor(
            modified_radial_build,
            original_elongation,
            original_triangularity,
            original_n_tf_coils,
            original_coil_height_factor,
        )
        export_reactor_to_png(reactor, f"tokamak_frame_{frame:03d}.png")
        frame += 1

for modified_n_tf_coils in [
    original_n_tf_coils,
    original_n_tf_coils - 1,
    original_n_tf_coils - 2,
    original_n_tf_coils - 3,
    original_n_tf_coils - 2,
    original_n_tf_coils - 1,
    original_n_tf_coils,
]:
...
os.system(
    "ffmpeg -r 10 -i tokamak_frame_%3d.png -c:v libx264 -r 30 -pix_fmt yuv420p tokamak_animation.mp4"
)
