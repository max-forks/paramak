...
my_model.export_dagmc_h5m_file(
    filename=h5m_filename, min_mesh_size=10.0, max_mesh_size=20.0
)
...
materials = openmc.Materials(
    [mat_layer_1, mat_layer_2, mat_layer_3, mat_layer_4, mat_layer_5, mat_concrete]
)
...
dagmc_geometry_offset = (
    abs(bbox[0][2]) + floor_vertical_thickness + reactor_to_floor_gap
)
...
bioshield_inner_surface = openmc.ZCylinder(
    r=dagmc_radius + bioshield_to_reactor_gap, surface_id=1002
)
...
bioshield_outer_surface = openmc.ZCylinder(
    r=dagmc_radius + bioshield_to_reactor_gap + bioshield_radial_thickness,
    boundary_type="vacuum",
    surface_id=1007,
)
...
geometry = openmc.Geometry(
    [dagmc_filled_cell, floor_cell, bioshield_cell, ceiling_cell]
)
...
mesh = openmc.CylindricalMesh(
    r_grid=np.linspace(
        0, dagmc_radius + bioshield_to_reactor_gap + bioshield_radial_thickness, 100
    ),
    z_grid=np.linspace(
        0,
