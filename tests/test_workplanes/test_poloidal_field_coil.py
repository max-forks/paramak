...
def test_construction():
    poloidal_field_coil = paramak.poloidal_field_coil(
        height=20, width=10, center_point=(100, 50), rotation_angle=360
    )
...
def test_volume_rotation_angle():
    ...
    assert math.isclose(
        half_poloidal_field_coil.val().Volume(), poloidal_field_coil.val().Volume() / 2
    )
