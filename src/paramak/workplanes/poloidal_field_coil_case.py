...
def poloidal_field_coil_case(
    ...
    inner_points = [
        (
            center_point[0] + coil_width / 2.0,
            center_point[1] + coil_height / 2.0,
            "straight",
        ),  # upper right
        (
            center_point[0] + coil_width / 2.0,
    ...
    inner_wire = create_wire_workplane_from_points(
        points=inner_points, plane=plane, origin=origin, obj=obj
    )
    outer_wire = create_wire_workplane_from_points(
        points=outer_points, plane=plane, origin=origin, obj=obj
    )
