...
def constant_thickness_dome(
    ...
    **kwargs,
    ...
    if not isinstance(chord_width, numbers.Number):
        raise ValueError(
            "ConstantThicknessDome.chord_width must be a float. Not", chord_width
        )
    if chord_width <= 0:
    ...
    if not isinstance(chord_height, numbers.Number):
        raise ValueError(
            "ConstantThicknessDome.chord_height must be a float. Not", chord_height
        )
    if chord_height <= 0:
    ...
    if thickness <= 0:
        msg = (
            f"VacuumVessel.thickness must be a positive number above 0. Not {thickness}"
        )
        raise ValueError(msg)
    ...
    if chord_height * 2 >= chord_width:
        msg = (
            "ConstantThicknessDome requires that the chord_width "
            "is at least 2 times as large as the chord height"
        )
        raise ValueError(msg)

    radius_of_sphere = (
        (math.pow(chord_width, 2)) + (4.0 * math.pow(chord_height, 2))
    ) / (8 * chord_height)
    ...
    if upper_or_lower == "upper":
        center_point = (
            chord_center[0],
            chord_center[1] + chord_height - radius_of_sphere,
        )
        inner_tri_angle = math.atan(
            (center_point[1] - chord_center[1]) / (chord_width / 2)
        )
        ...
        point_6 = (
            chord_center[0],
            chord_center[1] + radius_of_sphere + thickness,
            "straight",
        )
        ...
        msg = (
            f'upper_or_lower should be either "upper"  or "lower". Not {upper_or_lower}'
        )
        raise ValueError(msg)
    ...
    if upper_or_lower == "upper":
        ...
        raise ValueError(
            "upper_or_lower argument must be set to either 'upper' or 'lower'"
        )

    big_sphere = (
        cq.Workplane(plane)
        .moveTo(center_point[0], center_point[1])
        .sphere(radius_of_sphere + thickness)
    )
    ...
    wire = create_wire_workplane_from_points(
        points=(
            ...
            (
                chord_center[0] + radius_of_sphere + thickness,
                chord_center[1],
                "straight",
            ),
    ...
    cutter = cutting_wedge(
        height=height, radius=radius, rotation_angle=rotation_angle, plane=plane
    )
